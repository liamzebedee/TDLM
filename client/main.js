...
// import 'color-thief/dist/color-thief.min.js';
...
Template.index.onCreated( function(){
	this._userLoading = this.subscribe('_userLoading');
});

Template.index.helpers({
	userLoaded: function() {
		return Template.instance()._userLoading.ready();
	},
})


const GREETINGS = ["G'day", "ello ello", "WAAZZZZUPPP", "wanna toke?"]


function changeBackgroundBasedOnDominantColourOfLatestTrack() {
	return new Promise((resolve, reject) => {
		Meteor.call('getLatestTrack', (err, res) => {
			if(err) {
				console.error(err)
				return reject(err)
			}
			...
			if(latestTrack.item.track.album.images.length) {
				...
				img.onload = () => {
					...
					let domColourRgb = colorThief.getColor(img);
					resolve(`rgb(${domColourRgb[0]}, ${domColourRgb[1]}, ${domColourRgb[2]})`);
				}
			}
		});
	})
...
Template.leaderboard.helpers({
	tracks() {
		// console.log(Tracks.find({ playlist: TLDM_PLAYLIST_ID }))
...
Template.track.events({
	...
	'mouseover .track-album'(event, instance) {
		var colorThief = new ColorThief();
		let domColourRgb = colorThief.getColor(event.target);
		Session.set('bgcolorFromTrackImg', `rgb(${domColourRgb[0]}, ${domColourRgb[1]}, ${domColourRgb[2]})`);
	}
})
